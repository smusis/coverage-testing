...
public class RunRandoop {
	...
	{	
		...
		File theDir = new File("Data/"+project+"/RandoopTests");
		...
		stdin.println("java -cp Data/jars/randoop.1.3.4.jar;" +
				"Data/"+project+"/RandoopClasses"+version+";" +
				"Data/"+project+"/RandoopJars/*;Data/"+project+"/RandoopJarsCopy/* " +
				"randoop.main.Main gentests " +
				"--classlist=Data/"+project+"/classList.txt " +
				"--timelimit 100 "+
				"--junit-output-dir=Data/"+project+"/RandoopTests");
	...
	public static void compileTestCases(String project,String version) throws IOException, InterruptedException
	{
		...
		if(version.equals("Newer")){
			stdin.println("javac -cp Data/jars/randoop.1.3.4.jar;" +
					"\"Data/"+project+"/RandoopClasses"+version+"\";" +
					"\"Data/"+project+"/RandoopJars/*\" " +
					"Data/"+project+"/RandoopTests/RandoopTest*.java");
		}
		if(version.equals("Older")){
			stdin.println("javac -cp \"Data/jars/randoop.1.3.4.jar\";" +
					"\"Data/"+project+"/RandoopClasses"+version+"\";" +
					"\"Data/"+project+"/RandoopJars/*\" " +
					"Data/"+project+"/RandoopTestsCommented/RandoopTest*.java");
		}
		// write any other commands you want here
		stdin.close();
		int returnCode = p.waitFor();
	...
	public static void runTestCasesNewer(String project,String version) throws IOException, InterruptedException
	{
		...
		File theDir = new File("Data/"+project+"/RandoopOutput");
		...
		stdin.println("for /l %x in (1,1,50) do java -classpath " +
				...
				"junit.textui.TestRunner --output-nonexec=true --capture-output RandoopTest > " +
				"\"Data\\"+project+"\\RandoopOutput\\randoop%x\"");
		// write any other commands you want here
		stdin.close();
		int returnCode = p.waitFor();
		System.out.println("Return code = " + returnCode);
	...
	public static void runTestCasesOlder(String project,String version,String bugId,String older) throws IOException, InterruptedException
	{
		...
		stdin.println("java -classpath Data/jars/randoop.1.3.4.jar;Data/cobertura-1.9.4.1/lib/*;" +
				"Data/"+project+"/CoberturaInstrumented;Data/"+project+"/RandoopClassesOlder;" +
				"Data/"+project+"/RandoopJars/*;Data/"+project+"/RandoopTestsCommented " +
				"-Dnet.sourceforge.cobertura.datafile=Data/"+project+"/cobertura.ser " +
				"junit.textui.TestRunner --capture-output RandoopTest > Data/"+project+"/randoopNewer");
		/*stdin.println("java -classpath " +
				"\"Data\\jars\\randoop.1.3.4.jar\";" +
	...
	public static void commentNonDetTests(String project,String version,String bugId) throws IOException, InterruptedException
	{
		...
		File theDir = new File("Data/"+project+"/RandoopTestsCommented");
		FileUtils.deleteDirectory(theDir);
		if (!theDir.exists()) {
			boolean result = theDir.mkdir();  
		}
		...
		for(int i=0;i<testSuite.size();i++){
			...
			File fileExists=new File("Data/"+project+"/RandoopTestsCommented/RandoopTest"+testSuite.get(i)+".java");
			if(fileExists.exists()){
				...
				br = new BufferedReader(new FileReader("Data/"+project+"/RandoopTests/RandoopTest"+testSuite.get(i)+".java")); 
			}
			...
			File file=new File("Data/"+project+"/RandoopTestsCommented/RandoopTest"+testSuite.get(i)+".java");
		...
		for(int i=0;i<fileList.size();i++)
		{
			if(fileList.get(i).toString().contains(".java")){
				...
				File destination=new File("Data/"+project+"/RandoopTestsCommented/"+splitName[splitName.length-1]);
	...
	//Generate report for cobertura
		public static void generateCobReport(String project,String version,String bugId,String older) throws IOException, InterruptedException
		{
			...
			stdin.println("\"Data\\cobertura-1.9.4.1\\cobertura-report.bat\" " +
					"--format xml --datafile=\"Data/"+project+"/cobertura.ser\" " +
					"--destination \"Data/"+project+"/CoberturaCoverage\"");
			...
			String covSource="Data/"+project+"/CoberturaCoverage/coverage.xml";
			String covDest="Data/CoberturaData/CoberturaCoverage/"+older+"_"+bugId+".xml";
	...
	public static void getFailedTests(String project,String bugId,String older) throws IOException, InterruptedException
	{
		...
		File file=new File("Data/CoberturaData/PassedTests/"+older+"_"+bugId+".txt");
		...
		final BufferedReader br = new BufferedReader(new FileReader("Data/"+project+"/randoopNewer")); 
