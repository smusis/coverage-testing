...
public class CheckoutCommits {
	...
	public static void checkCommits() throws IOException, InterruptedException {
		...
		while ((line = br.readLine()) != null) {
			...
			if(!bugId.equals("JCR-2515")){ continue; }

			if (!project.equals("jackrabbit")) {
				continue;
			}

			boolean filePresent=false;
			File file=new File("Data/CoberturaData/GeneratedTestCases");
	...
	public static void deleteOldFolders(String project) throws IOException{
		...
		for(int i=0;i<folders.length;i++){
			...
			File theDir=new File("Data/"+project+"/"+folders[i]);
	...
	public static void checkoutCommit(String commit, String project)
		...
		// Variables on server
		//stdin.println("set PATH=%PATH%;E:\\Program^ Files\\apache-maven-3.2.1\\bin");
		//stdin.println("set JAVA_HOME=\"E:/Progra~1/Java/jdk1.7.0_25\"");
	...
	public static void findPackageNames(String project, String version)
		...
		File theDir = new File("Data/" + project + "/RandoopClasses" + version);
		...
		File folder = new File("Data/" + project + "/");
	...
	public static void copyClasses(ArrayList<String> packageNames,
		...
		File file = new File("Data/" + project + "/classList.txt");
		...
		for (String str : packageNames) {
			...
			for (int i = 0; i < split.length; i++) {
				...
				File theDir = new File("Data/" + project + "/RandoopClasses"
						+ version + "/" + folder);
		...
		for (String str : classFiles) {
			if (!(str.contains("\\test\\")||(str.contains("\\test-framework\\"))||(str.contains("\\test-classes\\")))) {
				...
				if (!packageName.isEmpty()) {
					...
					String fileName = "Data/" + project + "/RandoopClasses"
							+ version + "/" + packageName;
	...
	InterruptedException {
		...
		File theDir = new File("Data/" + project + "/RandoopJars");
		...
		for (String str : classFiles) {
			...
			String fileName = "Data/" + project + "/RandoopJars/" + jarName;
	...
	public static void instrumentCobertura(String project, String version)
		...
		File theDir = new File("Data/" + project + "/CoberturaInstrumented");
		...
		stdin.println("\"Data/cobertura-1.9.4.1/cobertura-instrument.bat\" "
				+ "\"Data/" + project + "/RandoopClasses" + version + "\""
				+ " --datafile \"Data/"+project+"/cobertura.ser\""
	...
	public static void copyAllData(String project, String bugId,String older)
		...
		File theDir = new File("Data/CoberturaData/GeneratedTestCases/"+older+"_"+bugId);
		// if the directory does not exist, create it
		if (!theDir.exists()) {
			boolean result = theDir.mkdir();
		}
		...
		File testExe = new File("Data/CoberturaData/TestExecutionsNewer/"+older+"_"+bugId);
		if (!testExe.exists()) {
			boolean result = testExe.mkdir();
		}

		folder=new File("Data/"+project+"/RandoopOutput/");
		...
		File testExeNew = new File("Data/CoberturaData/TestExecutionsOlder");
		if (!testExeNew.exists()) {
			boolean result = testExeNew.mkdir();
		}
		File source=new File("Data/"+project+"/randoopNewer");
